...
const setsFieldOption = makeSetsOptionMacro(getInstance, nameFieldExpectStrategy('geo_shape'));
...
test(setsFieldOption, 'shape', {
    param: new GeoShape().type('envelope').coordinates([[13.0, 53.0], [14.0, 52.0]])
});
test(setsFieldOption, 'indexedShape', {
    param: new IndexedShape().id('DEU').type('countries').index('shapes').path('location')
});
